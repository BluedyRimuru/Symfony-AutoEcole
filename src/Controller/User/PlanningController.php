...
{
    #[Route('/user/planning', name: 'app_user_planning')]
    public function index(Request $request, LeconRepository $leconRepository): Response
    {
        $rdvs = [];
        ...
        foreach ($leconRepository->getPlanning($this->getUser()->getId()) as $lecon) {
            ...
            foreach ($lecon->getEquipe() as $item ){
                if ($item->getId() != $this->getUser()->getId()){
                    $moniteur = $item->getNom() . ' ' . $item->getPrenom();
                }
            }
        ...
        return $this->render('user/planning/index.html.twig', compact('data'));
    }
}